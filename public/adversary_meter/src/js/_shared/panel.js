...
export class Panel extends com.computablefacts.widgets.Widget {
        ...
        if (form){
            form.addEventListener('submit', async (event) => {
                ...
                const addPromises = alertsToAdd.map(option => {
                    const payload = {};
                    if (option === 'uid') payload.uid = this.selectedVuln_.uid;
                    if (option === 'type') payload.type = this.selectedVuln_.fullType;
                    if (option === 'title') payload.title = this.selectedVuln_.title2;
                ...
                const alertsToRemove = Object.keys(existingAlertsMap).filter(option => existingAlertsMap[option] && !selectedOptions.includes(option));
    ...
    _newBody(infos) {
        ...
        return `
            ...
            {date: infos.modifications[0].timestamp, user: infos.modifications[0].user})}
                  </div>
                </div>
            </div>
    ...
    _newSelectedVuln(ports, hiddenAlerts) {
        ...
        const isCategoryHidden = (category) => {
            ...
            if (category === 'type') vulnProperty = 'fullType';
            if (category === 'title') vulnProperty = 'title2';
            ...
            rows = ports.map((port, index) => {
                ...
                return `
                    <tr class="${index % 2 === 0 ? 'row-light' : ''}">
                      ...
                      <td class="text-truncate" style="width:100px" title="${port.products[0] ? port.products[0] : '-'}">${port.products[0] ? port.products[0] : '-'}</td>
    ...
    _newAlerts(ports, vulnerabilities, hiddenAlerts) {
        ...
        const displayedVulnerabilities = vulnerabilities.filter(vuln => {
            return !hiddenAlerts.some(hiddenAlert => {
                return vuln.uid === hiddenAlert.uid ||
                  vuln.type === hiddenAlert.type ||
                  vuln.title === hiddenAlert.title;
            });
        });
    ...
    _newTimeline(timeline) {
        ...
        function createTimelineItem(taskName, taskData, isNextScan = false) {
            ...
            const end = taskData.end ? moment(taskData.end).format('YYYY-MM-DD HH:mm:ss UTC') : i18next.t(
                'en cours...');
